...
public class Drivetrain {
    ...
    public void drive() {
        ...
        if (control1.getBButton()) {
            Drive(errorAdjust, -errorAdjust);
        }
        else {
            Drive(-control1.getLeftY()*.90, -control1.getRightY()*.90);
        }
    ...
    {
      dt_main.tankDrive(l, r);  
    }

        // Util methods
    public void Limelight(double y_off)
    {
       if (( tx > 1.5 || tx < -1.5) ) {
            double errorAdjust = NetworkTableInstance.getDefault().getTable("limelight").getEntry("tx").getDouble(0);
            errorAdjust *= 0.05;
